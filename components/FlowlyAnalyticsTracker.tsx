...
function inferFunnelStep(path: string, fullPath = path) {
  ...
  if (cleanPath.includes("precios") || cleanPath.includes("pricing") || cleanPath.includes("#precios")) return "pricing";
...
export default function FlowlyAnalyticsTracker() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        await fetch("/api/analytics/track", {
          ...
          body: JSON.stringify({
            ...
            funnelStep: inferFunnelStep(window.location.pathname || pathname, nextFullPath || fullPath),
    ...
    const handleClick = (event: MouseEvent) => {
      const target = event.target instanceof Element ? event.target.closest("a,button,[data-track-label]") : null;
      if (!target) return;
      ...
      send("cta_click", {
        ...
        funnelStep: isCheckout ? "checkout" : isSignup ? "signup" : isPricing ? "pricing" : inferFunnelStep(window.location.pathname),
      });
    ...
    const handleScroll = () => {
      ...
      const nextDepth = SCROLL_DEPTHS.find((depth) => percent >= depth && !sentScrollDepthsRef.current.has(depth));
      if (!nextDepth) return;
