...
export function generateModuleFromArtifacts(moduleName: string, artifacts: FlowlyStudioStoredArtifact[]): FlowlyModuleGeneration {
  ...
  const apiManifest = JSON.stringify({
    ...
    note: "Generated routes must execute through Capability Runtime and domain contracts.",
  }, null, 2);

  const sdkIndex = generated.map(({ artifact, generated }) => `// ${artifact.name}\n${generated.sdk}`).join("\n\n");
  const docsIndex = `# ${moduleName}\n\nGenerated by Flowly Studio.\n${section("Artifacts", artifacts.map((artifact) => `- **${artifact.name}** (${artifact.kind}) — ${artifact.description}`).join("\n"))}${section("Architecture Score", `${analysis.architectureScore}/100 · Risk: ${analysis.riskLevel}`)}${section("Warnings", analysis.warnings.length ? analysis.warnings.map((warning) => `- ${warning}`).join("\n") : "- No warnings detected.")}${section("Recommendations", analysis.recommendations.length ? analysis.recommendations.map((item) => `- ${item}`).join("\n") : "- Ready for review.")}`;
  const testPlan = `# ${moduleName} Test Plan\n\n${generated.map(({ artifact, generated }) => `## ${artifact.name}\n\n${generated.testPlan}`).join("\n\n")}\n\n## Architecture Tests\n\n- Apps must consume Capabilities only.\n- Capabilities must use Commands/Queries.\n- Business Objects must emit Events.\n- Generated artifacts must be reviewed before production.`;
  const impactReport = `# ${moduleName} Impact Report\n\n- Architecture Score: ${analysis.architectureScore}/100\n- Risk Level: ${analysis.riskLevel}\n- Nodes: ${analysis.nodes.length}\n- Edges: ${analysis.edges.length}\n\n## Totals\n\n${Object.entries(analysis.totals).map(([kind, total]) => `- ${kind}: ${total}`).join("\n")}\n\n## Warnings\n\n${analysis.warnings.length ? analysis.warnings.map((warning) => `- ${warning}`).join("\n") : "- None"}\n\n## Recommendations\n\n${analysis.recommendations.length ? analysis.recommendations.map((item) => `- ${item}`).join("\n") : "- None"}`;

  return {
    id: `generation_${slug}_${Date.now()}`,
    moduleName,
    slug,
    summary: `Generated ${artifacts.length} artifacts for ${moduleName}. Architecture score ${analysis.architectureScore}/100.`,
    artifacts,
    migrationSql,
    typeScriptIndex,
    apiManifest,
