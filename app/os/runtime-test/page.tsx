...
export default function RuntimeTestPage() {
  ...
  const statusTone = useMemo(() => {
    if (snapshot.phase === "error") return "text-rose-200";
    if (snapshot.phase === "recording" || snapshot.phase === "transcribing") return "text-cyan-200";
    if (snapshot.phase === "speaking" || snapshot.phase === "thinking") return "text-amber-200";
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <section className="rounded-2xl border border-white/10 bg-black/20 p-5">
            ...
            <div className={`mt-3 text-sm ${statusTone}`}>
              ...
              <div><strong>Engine:</strong> {snapshot.phase}</div>
          ...
          <section className="rounded-2xl border border-white/10 bg-black/20 p-5">
            ...
            <div className="mt-3 space-y-2 text-sm text-white/70">
              <div><strong>Última petición:</strong> {snapshot.lastBrainRequest || "—"}</div>
              <div><strong>Última respuesta:</strong> {snapshot.lastBrainResponse || "—"}</div>
