        ...
        {tab === "leads" && (
          <section className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
            <Panel title="Nuevo lead">
              <div className="grid gap-3"><input value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Empresa / negocio" className="input-dark" /><div className="grid gap-3 md:grid-cols-2"><input value={contactName} onChange={(e) => setContactName(e.target.value)} placeholder="Persona de contacto" className="input-dark" /><select value={sector} onChange={(e) => setSector(e.target.value)} className="input-dark"><option>Peluquería</option><option>Barbería</option><option>Estética</option><option>Clínica</option><option>Academia</option><option>Restaurante</option><option>Otros</option></select></div><div className="grid gap-3 md:grid-cols-2"><input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Teléfono" className="input-dark" /><input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" className="input-dark" /></div><textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Notas comerciales" className="input-dark min-h-24" /><button onClick={createLead} className="rounded-full bg-gradient-to-r from-cyan-300 to-violet-300 px-5 py-3 font-semibold text-slate-950 shadow-lg shadow-cyan-500/20"><Plus size={18} className="inline" /> Crear lead</button></div>
            </Panel>
