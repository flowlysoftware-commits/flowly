...
function uniqueSessions(events: EventRow[], predicate: (row: EventRow) => boolean) {
  return new Set(events.filter(predicate).map((row) => row.session_id).filter(Boolean)).size;
}

function uniqueVisitors(events: EventRow[], predicate: (row: EventRow) => boolean) {
  return new Set(events.filter(predicate).map((row) => row.visitor_id).filter(Boolean)).size;
}
...
export async function GET(request: NextRequest) {
  ...
  const [online, eventsToday, visitorsToday, allEvents, sessions30, recentEvents] = await Promise.all([
    ...
    supabaseAdmin.from("flowly_analytics_events").select("id", { count: "exact", head: true }).gte("created_at", today),
    ...
    supabaseAdmin
      ...
      .select("event_name, path, full_path, funnel_step, visitor_id, session_id, created_at, duration_ms, viewport, language, metadata")
    ...
    supabaseAdmin.from("flowly_analytics_events").select("event_name, path, funnel_step, created_at, viewport, language, metadata").order("created_at", { ascending: false }).limit(35),
  ]);
  ...
  const funnel = funnelOrder.map((step, index) => {
    const count = uniqueSessions(events, (row) => row.funnel_step === step || (step === "pricing" && row.event_name === "cta_click" && String(row.metadata?.href || "").includes("precios")));
    const previousCount = index === 0 ? count : uniqueSessions(events, (row) => row.funnel_step === funnelOrder[index - 1]);
    const dropRate = index === 0 || previousCount === 0 ? 0 : Math.max(0, Math.round(((previousCount - count) / previousCount) * 100));
  ...
  const campaignMap = new Map<string, { source: string; campaign: string; sessions: Set<string>; ctaClicks: number; signupStarts: number }>();
  for (const event of events) {
    const { source, campaign } = readAttribution(event);
    const key = `${source} · ${campaign}`;
    const row = campaignMap.get(key) || { source, campaign, sessions: new Set<string>(), ctaClicks: 0, signupStarts: 0 };
  ...
  const campaigns = Array.from(campaignMap.values())
    .map((row) => ({ source: row.source, campaign: row.campaign, sessions: row.sessions.size, ctaClicks: row.ctaClicks, signupStarts: row.signupStarts }))
    .sort((a, b) => b.sessions - a.sessions)
    .slice(0, 8);

  const ctaMap = new Map<string, number>();
  for (const event of events.filter((row) => row.event_name === "cta_click")) {
    const label = String(event.metadata?.label || event.metadata?.href || "CTA sin etiqueta");
  ...
  const pricingSessions = uniqueSessions(events, (row) => row.funnel_step === "pricing");
  const signupStartedSessions = uniqueSessions(events, (row) => row.event_name === "signup_started" || row.funnel_step === "signup");
  const ctaClickSessions = uniqueSessions(events, (row) => row.event_name === "cta_click");
  const scroll75Sessions = uniqueSessions(events, (row) => row.event_name === "scroll_depth" && Number(row.metadata?.depth || 0) >= 75);
  ...
  const summary = {
    ...
    pricingRate: landingSessions ? Math.round((pricingSessions / landingSessions) * 100) : 0,
    ctaRate: landingSessions ? Math.round((ctaClickSessions / landingSessions) * 100) : 0,
    signupRate: landingSessions ? Math.round((signupStartedSessions / landingSessions) * 100) : 0,
    checkoutConversion: checkoutSessions ? Math.round((purchaseSessions / checkoutSessions) * 100) : 0,
  };

  const recommendations = [];
  if (summary.landingSessions >= 20 && summary.ctaRate < 8) {
    recommendations.push(`Problema principal: mucha gente llega a la landing, pero solo el ${summary.ctaRate}% hace clic. Hay que mejorar promesa, CTA y confianza arriba del todo.`);
  }
  if (summary.landingSessions >= 20 && summary.pricingRate < 10) {
    recommendations.push(`Solo el ${summary.pricingRate}% avanza a precios. La publicidad está trayendo visitas, pero la home no está empujando suficiente al siguiente paso.`);
  }
  if (summary.ctaClickSessions > 0 && summary.signupStartedSessions === 0) {
    recommendations.push("Hay clics en CTA, pero no empieza el registro. Revisa que el botón lleve a la ruta correcta y que el formulario cargue bien en móvil.");
  }
