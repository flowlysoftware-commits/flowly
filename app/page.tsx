          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.06] p-4">
            <div className="flex h-40 items-end gap-2">
              {[35, 52, 44, 80, 62, 94, 72, 88].map((h, i) => (
                <div
                  key={i}
                  className="flex-1 rounded-t-xl bg-gradient-to-t from-violet-500 to-pink-300"
                  style={{ height: `${h}%` }}
                />
              ))}
            </div>
    ...
    <main className="min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,#f3e8ff_0%,#ffffff_35%,#f8fafc_100%)] text-neutral-950">
      ...
      <section id="demos" className="mx-auto max-w-7xl px-6 py-20">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {sectors.map((sector) => (
            <div
              key={sector.name}
              className="rounded-[2rem] border border-neutral-200 bg-white/80 p-6 shadow-sm backdrop-blur transition hover:-translate-y-1 hover:shadow-premium"
            >
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                {sector.stats.map((stat) => (
                  <div
                    key={stat}
                    className="rounded-2xl bg-neutral-50 p-4 text-sm font-medium text-neutral-700"
                  >
                    {stat}
                  </div>
                ))}
      ...
      <section id="features" className="mx-auto max-w-7xl px-6 py-20">
        <div className="mb-10 text-center">
          ...
          <h2 className="mt-3 text-4xl font-semibold tracking-tight">
            Todo lo necesario para gestionar y automatizar.
          </h2>
        </div>
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {features.map(({ Icon, title, text }) => (
            <div
              key={title}
              className="rounded-[1.5rem] border border-neutral-200 bg-white/75 p-6 shadow-sm backdrop-blur"
            >
